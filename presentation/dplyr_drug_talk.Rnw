...
select(playerID) %>% 
summarise(n_distinct(playerID))        # n_distinct is a fast length(unique())
...
dplyr_inner %>% summarise(n_distinct(playerID))
...
# the semi join returns a subset of the person_df data.frame which has at least
...
# there are 83 - 54 = 29 players who have no pitching stats... who are they?
...
# For each player, the proportion of years with more home runs than the prior
# year
batting %>% 
  group_by(playerID) %>%
...
-2 * log((1/sqrt(2 * pi * sigma^2))^(N) * exp(-1/(2 * sigma^2) * sum((y - mu)^2)))
